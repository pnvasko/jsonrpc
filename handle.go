...
func (s *Server) Handle(ctx context.Context, sock Socket) {
	...
	for req := range readRequests(ctx, sock) {
		...
		go dispatch(ctx, s.rcvr, req, responses, s.methods)
	}
}

func dispatch(ctx context.Context, rcvr interface{}, req *Request, responses chan<- *Response, methods Methods) {
